...
(() => {
  ...
  function radarJob() {
    const svg = $('.ch');
    if (!svg) return null;
    const grow  = $('.ch__scale', svg);
    const sonar = $('.ch__sonar', svg);
    const rings = $$('.ch__sonar circle', svg);
    if (!grow || !sonar || !rings.length) return null;

    const RMAX = parseFloat(sonar.dataset.rmax) || 210;
    /* Mittelpunkt der Zeichenfläche — darum herum wird vergrössert */
    const vb = (svg.getAttribute('viewBox') || '0 0 100 100').split(/\s+/).map(Number);
    const cx = vb[0] + vb[2] / 2, cy = vb[1] + vb[3] / 2;

    const sec = $('#gebiet') || svg;
    let top = 0, h = 1;
    ...
    return {
      ...
      draw(v) {
        /* Wie weit der Abschnitt hereingekommen ist */
        const ein = clamp((v.y + view.h - top) / (view.h * 0.85));
        const w = ease(ein);

        /* Die Karte wächst und klart auf.
           Als Attribut gesetzt, damit keine CSS-Bezugsrahmen nötig sind. */
        const s = 0.82 + 0.18 * w;
        grow.setAttribute('transform',
          `translate(${cx.toFixed(1)} ${cy.toFixed(1)}) scale(${s.toFixed(4)}) ` +
          `translate(${(-cx).toFixed(1)} ${(-cy).toFixed(1)})`);
        grow.style.opacity = (0.25 + 0.75 * w).toFixed(3);

        /* Erst wenn die Karte steht, beginnt das Echolot */
        if (ein < 0.985) {
          for (const r of rings) { r.setAttribute('r', '0'); r.style.opacity = '0'; }
          return;
        }
        const nach = clamp((v.y + view.h - top - view.h * 0.85) / (h + view.h * 0.15));
        const takt = nach * 3.2;

        for (let i = 0; i < rings.length; i++) {
          const t = (takt - i * 0.33) % 1;
          if (t < 0) { rings[i].setAttribute('r', '0'); rings[i].style.opacity = '0'; continue; }
          rings[i].setAttribute('r', (t * RMAX).toFixed(1));
          /* rasch da, langsam verklingend */
          rings[i].style.opacity =
            ((t < 0.08 ? t / 0.08 : 1 - (t - 0.08) / 0.92) * 0.8).toFixed(3);
        }
      }
    };
  }
  ...
  /* ====================================================================
     6c — Rückfallebene für das Enthüllen
     Der Beobachter ist der Normalweg. Zusätzlich prüft die Scroll-Schleife,
     ob noch etwas verdeckt ist, das längst im Bild steht. Sie läuft ohnehin
     für die Bühne — damit kann nichts dauerhaft unsichtbar bleiben.
     ==================================================================== */
  function revealJob() {
    let rest = $$('.rv, .rv-line, .rv-wipe, .flow__step, .craft__item, .tariff__card, .zone, .creed li')
    ...
    return {
      measure() {},
      draw() {
        if (++tick % 12) return;            /* alle zwölf Bilder genügt */
        if (!rest.length) return;
        const bleibt = [];
        for (const el of rest) {
          if (el.classList.contains('is-in')) continue;
          const r = el.getBoundingClientRect();
          if (r.top < view.h * 0.94 && r.bottom > 0) el.classList.add('is-in');
          else bleibt.push(el);
        }
        rest = bleibt;
      }
  ...
  function boot() {
    ...
    [stageJob, headJob, spoolJob, parallaxJob, tickerJob, spineJob, radarJob, revealJob]
      .map(f => f())
      .filter(Boolean)
